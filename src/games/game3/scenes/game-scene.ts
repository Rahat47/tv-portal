...
export class GameScene extends Phaser.Scene {
  ...
  update(): void {
    if (!this.bird.getDead()) {
      ...
      let arcadePhysicsCallback: ArcadePhysicsCallback = () => {};
      this.physics.overlap(
        this.bird,
        this.pipes,
        function () {
          self.bird.setDead(true);
        },
        arcadePhysicsCallback,
        this
      );
    } else {
      let callCallback: CallCallback = () => {
        //pipe.body.setVelocityX(0);
      };
      Phaser.Actions.Call(this.pipes.getChildren(), callCallback, this);
